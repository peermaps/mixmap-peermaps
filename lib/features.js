...
var ConcurrentDecoder = require('./concurrent-decoder')
...
function Features(opts) {
  ...
  if (!opts) opts = {}
  if (typeof opts.decoderOnItem !== 'function') opts.decoderOnItem = noop

  this.decoder = ConcurrentDecoder({
    count: 4,
    itemId: function (row) {
...
Features.prototype.addBuffer = function (queryIndex, buffer) {
  ...
  this.addBufferDecoded({ queryIndex, buffer, decoded })
}
